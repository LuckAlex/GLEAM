...
from NURBSAttacker import Attacker, ImageAttacker, TextAttacker
...
def retrieval_eval(model, ref_model, t_models, t_ref_models, t_test_transforms, data_loader, tokenizer, t_tokenizers, device, args,config):
    ...
    img_attacker = ImageAttacker(model, images_normalize, model_name='', epsilon=8/255, alpha=6/255, epoch=10, decay=1.,
                                  mesh_width=30, mesh_height=30, rho=100, num_warping=20, noise_scale=2, beta=1.5,
                                  num_neighbor=20, targeted=False, random_start=False, norm='linfty',
                                  loss='crossentropy', device=device, attack='NURBS_Adaptive')
    ...
    txt_attacker = TextAttacker(ref_model, tokenizer, cls=False, max_length=max_length, number_perturbation=1,
                                topk=10, threshold_pred_score=0.3, device=device)
